...
const AddStock: FC = () => {
  ...
  const { data: tickerInfo, error: tickerErrorQuery } = useGetTickerInfoQuery(
    tickerInput.toUpperCase(),
    {
      skip: !isQueryEnabled,
  ...
  const { data: stockPriceInfo, error: stockPriceErrorQuery } =
    useGetStockPricesForDateQuery(
      {
            ...
            if (!result.isFollowing) {
              const newTicker = {
                ...
                previousDayStockPrice:
                  stockPriceInfo.results[stockPriceInfo.results.length - 1].c,
              };
  ...
  return (
    <div>
      <div className="flex justify-between items-center">
        ...
        <Link to="/" className="base-btn m-2">
          Back to Main
        </Link>
      </div>
      <TickerInput
      ...
      <div>
        {following && !hasError && (
          <p> You are already tracking the ticker from the date {trackeDate}</p>
        )}
