...
const StockDetails: FC = () => {
  ...
  const {
    ...
    refetch,
  } = useGetStockPricesForDateQuery({ ticker, endDate, startDate });
  ...
  const previousDayStockPrice = tickerExists
    ? tickers.find((t) => t.ticker === ticker)?.previousDayStockPrice ?? null
    : null;
  ...
  return (
    <div>
      ...
      {stockPricesError && (
        <p>
          Error loading stock prices for the last year. Try again in a minute!
        </p>
      )}
      {stockPrices && (
        <div>
          ...
          <StockChart
            data={stockPrices.results.map((price: StockPriceItem) => price.c)}
            previousDayStockPrice={previousDayStockPrice}
          />
        </div>
      )}
    </div>
