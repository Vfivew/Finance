...
const AddStock: React.FC = () => {
  ...
  const { data: tickerInfo, error: tickerError } = useGetTickerInfoQuery(tickerInput.toUpperCase(), {
    skip: !isQueryEnabled,
  ...
  useEffect(() => {
    if (tickerInfo && tickerInfo.results && stockPriceInfo.results) {
      setIsLoading(true); // Устанавливаем состояние загрузки при начале загрузки данных
      addDataFirebase({ results: tickerInfo.results, userEmail, selectedDate, stockPrice: stockPriceInfo.results[0].c })
        ...
        .finally(() => {
          setIsLoading(false); // Устанавливаем состояние загрузки в false после завершения загрузки
        });
    }
      ...
      <div>
        ...
        <button onClick={handleGetTickerInfo}>Получить информацию</button>
          <div>
            ...
            {stockPriceError ? (
              <p>Ошибка при запросе: Произошла ошибка при получении информации о цене.</p>
            ) : (
              stockPriceInfo && stockPriceInfo.results && stockPriceInfo.results.length > 0 ? (
                <div>
                  <h2>Информация по цене числа {selectedDate}:</h2>
                  {isLoading ? ( // Показываем заголовок и данные цены только если не загружаем
                    <Loading />
                  ) : (
                    <div>
        ...
        {tickerError ? (
          <p>Ошибка при запросе: Произошла ошибка при получении информации о тикере.</p>
        ) : (
          tickerInfo && (
            <div>
              <div>
                <h2>Информация по тикеру {tickerInput}:</h2>
                {isLoading ? ( // Показываем заголовок и данные тикера только если не загружаем
                  <Loading />
                ) : (
                  <div>
